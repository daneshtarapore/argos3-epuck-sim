/**
 * @file <argos3/plugins/simulator/sensors/light_updated_default_sensor.cpp>
 *
...
#include "light_updated_default_sensor.h"
...
namespace argos {
   ...
   static CRadians SENSOR_SPACING      = CRadians(ARGOS_PI / 8.0f);
   ...
   CLightUpdatedDefaultSensor::CLightUpdatedDefaultSensor() :
   ...
   void CLightUpdatedDefaultSensor::SetRobot(CComposableEntity& c_entity) {
      try {
   ...
   void CLightUpdatedDefaultSensor::Init(TConfigurationNode& t_tree) {
      try {
         CCI_LightUpdatedSensor::Init(t_tree);
         /* Show rays? */
         ...
         //m_tReadings.resize(m_pcLightEntity->GetNumSensors()); fixed to 8 in control interface for this sensor
      }
      catch(CARGoSException& ex) {
   ...
   void CLightUpdatedDefaultSensor::Update()
   {
//      /* Erase readings */
//      for(size_t i = 0; i < m_tReadings.size(); ++i)  m_tReadings[i].Value = 0.0f;
...
//      }
         ...
         if(cLight.GetIntensity() > 0.0f) {
            ...
            if(! GetClosestEmbodiedEntityIntersectedByRay(sIntersection,
               ...
               for(SInt32 nIndexOffset = -2; nIndexOffset < 3; ++nIndexOffset) {
                  UInt32 unIdx = Modulo(nReadingIdx + nIndexOffset, m_tReadings.size());
   ...
   void CLightUpdatedDefaultSensor::Reset() {
   ...
   Real CLightUpdatedDefaultSensor::CalculateReading(Real f_distance, Real f_intensity) {
   ...
   REGISTER_SENSOR(CLightUpdatedDefaultSensor,
                   "light_updated", "default",
                   "Danesh Tarapore [daneshtarapore@gmail.com]",
                   "1.0",
                   "A generic light sensor.",
                   "This sensor accesses a set of light sensors. It has been updated so that the angle of sensors is stored with the values - making computation of heading vector easy.   The sensors all return a value\n"
