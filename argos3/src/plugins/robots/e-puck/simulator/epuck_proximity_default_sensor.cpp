...
namespace argos {
   ...
   void CEPuckProximityDefaultSensor::Init(TConfigurationNode& t_tree) {
      try {
         ...
         //m_tReadings.resize(m_pcProximityEntity->GetNumSensors()); // the angles of the sensors have been set - we dont want to change them
      }
      catch(CARGoSException& ex) {
   ...
   void CEPuckProximityDefaultSensor::Update()
   {
      ...
      for(UInt32 i = 0; i < m_tReadings.size(); ++i) {
         ...
         if(GetClosestEmbodiedEntityIntersectedByRay(sIntersection,
            ...
            m_tReadings_Noiseless[i].Value = m_tReadings[i].Value;
         }
         else {
            /* No intersection */
            m_tReadings[i].Value = 0.0f;
            m_tReadings_Noiseless[i].Value = 0.0f;
         ...
         if(m_bAddNoise)
         {
            ...
            //std::cerr << " m_pcRNG->Uniform(m_cNoiseRange) " << m_pcRNG->Uniform(m_cNoiseRange) << std::endl;
         }
         /* Trunc the reading between 0 and 1 */
         UNIT.TruncValue(m_tReadings[i].Value);
         UNIT.TruncValue(m_tReadings_Noiseless[i].Value);
      }
   }
